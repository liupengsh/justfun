...
public class NotifyMissed {
	...
	public static void main(String[] args) {
		final NotifyMissed mn = new NotifyMissed();
		Runnable runA = new Runnable() {
			public void run() {
				try {
					// 休眠1000ms,大于runB中的500ms,
					// //是为了后调用waitToProceed,从而先notifyAll,后wait, //从而造成通知的遗漏
					Thread.sleep(1000);
					mn.waitToProceed();
		...
		Runnable runB = new Runnable() {
			public void run() {
				try {
					...
					// //是为了先调用proceed,从而先notifyAll,后wait, //从而造成通知的遗漏
					Thread.sleep(500);
					mn.proceed();
				} catch (InterruptedException x) {
					x.printStackTrace();
